...
# Test focus epic preference
set -e

# Create a test todo with two epics and tasks
cat > test_focus_todo.json << 'EOF'
{
  "version": "2.3.0",
  "project": {
    "name": "test",
...
echo "Testing with focus on T001 (Epic 1)..."
echo "Expected: Should prefer T002 (task in Epic 1) over T004 (task in Epic 2)"
echo ""

TODO_FILE=test_focus_todo.json bash scripts/next.sh --explain --format text